...
if(strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'mobile') || strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'android')) {
	...
	if (isset($_GET['mas']) && $_GET['mas'] != '') {
		// mas => mobile aspect ratio
		$mas_ = explode(':',$_GET['mas']);
		$mas = $mas_[0]/$mas_[1];
		if ($mas > 0) {
			$mas = floatval($mas);
			$maxWidth = 0;
...
{
	if (!$itsMobile || ($itsMobile && !isset($_GET['mas']))) {
...
if ($itsMobile && isset($_GET['mas'])) $_GET['cropratio'] = $_GET['mas'];
